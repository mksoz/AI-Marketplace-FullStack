...
const ClientMessages: React.FC = () => {
   ...
   const filteredChats = chats.filter(c => {
      ...
      const matchesSearch = c.counterpartyName.toLowerCase().includes(searchQuery.toLowerCase()) ||
         ...
         <div className="flex h-full w-full bg-white relative overflow-hidden">
            ...
            <aside className={`w-full md:w-80 lg:w-96 border-r border-gray-200 flex flex-col bg-white shrink-0 ${showChatOnMobile ? 'hidden md:flex' : 'flex'}`}>
               <div className="p-4 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                  <h2 className="font-bold text-gray-900 text-xl tracking-tight">Mensajes</h2>
                        ...
                        >
                           ...
                           <div className="flex-1 min-w-0 flex flex-col justify-center">
                              <div className="flex justify-between items-baseline mb-1">
                                 <h3 className="font-bold text-[15px] text-gray-900 truncate pr-2">{chat.counterpartyName}</h3>
                                 <span className="text-[10px] font-medium text-gray-400 flex-shrink-0">{new Date(chat.updatedAt).toLocaleDateString([], { day: '2-digit', month: '2-digit' })}</span>
                              </div>
                     ...
                     <header className="px-4 md:px-6 py-3 bg-[#f0f2f5] border-b border-gray-200 flex justify-between items-center z-20 shadow-sm">
                        <div className="flex items-center gap-3">
                           ...
                           <div className="min-w-0">
                              <h2 className="font-bold text-gray-900 leading-tight truncate">{selectedChat.counterpartyName}</h2>
                              <p className="text-[11px] text-gray-500 font-medium truncate italic">{selectedChat.projectTitle}</p>
                           </div>
                        </div>
            ...
            <div className="space-y-6 p-4">
               <div className="flex items-center gap-4 bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                  <div className="bg-white p-2 rounded-lg shadow-sm">
                     <span className="material-symbols-outlined text-indigo-600 text-2xl">videocam</span>
                  </div>
                  <div>
